var error_state_Order_Detail = false;

function creatmodalOrderDetail(Content) {
...
function creatformOrderDetail(content, listtag, listbuttons, title) {
    var modal = creatmodalOrderDetail(content);

    modal.Header.innerHTML += '<h4>' + title + '<h4>';

    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagOrderDetail(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagOrderDetail(type, title, name, event) {
    ...
    if (name == 'ProductName') {
        tag = ' <input disabled class="form-control" name= "' + name + '" id= "' + name + '"  placeholder= "' + title + ' را وارد کنید " >';

    }
    var str1 = "";
    if (error_state_Order_Detail == false) {
