var error_state_UserRegisterProduct = false;
function creatmodalUserRegisterProduct(Content) {
...
function creatformUserRegisterProduct(content, listtag, listbuttons, title) {
    var modal = creatmodalUserRegisterProduct(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagUserRegisterProduct(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagUserRegisterProduct(type, title, name, event) {
    ...
    var str1 = "";
    if (error_state_UserRegisterProduct == false) {
        ...
        error_state_UserRegisterProduct = true;
    }
