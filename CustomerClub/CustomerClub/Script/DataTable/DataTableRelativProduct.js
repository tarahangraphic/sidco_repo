function DataTable(ListData, colums, Content, option, buttons) {
    if (ListData == null || ListData.length == 0) {
        return "";
    }
    var _table = '<table class="table table-striped table-border table-hover "><thead><tr>';
    if (option.RowNumber) {
        _table += '<th>ردیف</th>';
    }
    for (var i in colums) {
    ...
    for (var i in ListData) {
        ...
        if (ListData[i].SalveProductName != "هیچ اطلاعاتی موجود نیست") {
            if (buttons != null) {
