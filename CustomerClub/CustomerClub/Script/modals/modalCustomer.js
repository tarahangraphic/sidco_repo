var error_state_Customer = false;
function creatmodalCustomer(Content) {
...
function creatformCustomer(content, listtag, listbuttons, title) {
    var modal = creatmodalCustomer(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagCustomer(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagCustomer(type, title, name, event) {
