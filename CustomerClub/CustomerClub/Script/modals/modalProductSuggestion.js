var error_state_ProductSuggestion = false;
function creatmodalProductSuggestion(Content) {
...
function creatformProductSuggestion(content, listtag, listbuttons, title) {
    var modal = creatmodalProductSuggestion(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagProductSuggestion(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagProductSuggestion(type, title, name, event) {
    ...
    var str1 = "";
    if (error_state_ProductSuggestion == false) {
        ...
        error_state_ProductSuggestion = true;
    }
