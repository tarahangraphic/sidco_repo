var error_state_Product_list = false;
function creatmodalListProduct(Content) {
...
function creatformProductList(content, listtag, listbuttons, title) {
    var modal = creatmodalListProduct(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagListProduct(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagListProduct(type, title, name, event) {
    ...
    var str1 = "";
    if (error_state_Product_list == false) {
