var error_state_Property = false;
function creatmodalProperty(Content) {
...
function creatformProperty(content, listtag, listbuttons, title) {
    var modal = creatmodalProperty(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagProperty(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagProperty(type, title, name, event) {
