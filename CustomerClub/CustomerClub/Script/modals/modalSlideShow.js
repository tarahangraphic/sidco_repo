var error_state_SlideShow = false;
function creatmodalSlideShow(Content) {
...
function creatformSlideShow(content, listtag, listbuttons, title) {
    var modal = creatmodalSlideShow(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagSlideShow(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagSlideShow(type, title, name, event) {
    ...
    var str1 = "";
    if (error_state_SlideShow == false) {
        ...
        error_state_SlideShow = true;
    }
    str1 += ' <div class="form-group"><label class="control-label col-sm-3" for="' + name + '">' + title + '</label><div class="col-sm-5">';
