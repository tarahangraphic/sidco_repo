var error_state_aboutus = false;

function creatmodalAboutUs(Content) {
...
function creatformAboutUs(content, listtag, listbuttons, title) {
    var modal = creatmodalAboutUs(content);

    modal.Header.innerHTML += '<h4>' + title + '<h4>';
 
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagAboutUs(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagAboutUs(type, title, name, event) {
