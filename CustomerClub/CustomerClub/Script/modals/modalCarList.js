var error_state_Car_list = false;

function creatmodalCarList(Content) {
...
function creatformListCar(content, listtag, listbuttons, title) {
    var modal = creatmodalCarList(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagCarList(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagCarList(type, title, name, event) {
