...
var error_state_Car_Product = false;

function creatmodalCarProduct(Content) {
...
function creatformCarProduct(content, listtag, listbuttons, title) {
    var modal = creatmodalCarProduct(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagCarProduct(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagCarProduct(type, title, name, event) {
    ...
    if (error_state_Car_Product == false) {
