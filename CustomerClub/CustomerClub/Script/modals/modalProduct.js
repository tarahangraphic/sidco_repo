var error_state_Product = false;
function creatmodalProduct(Content) {
...
function creatformProduct(content, listtag, listbuttons, title) {
    var modal = creatmodalProduct(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagProduct(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagProduct(type, title, name, event) {
    ...
    if (type == 'file') {
        tag = '<input type="file" id="' + name + '" name="' + name + '"/><img id="Img1" src="" class="PicMember" />';
    }
    
    var str1 = "";
    if (error_state_Product == false) {
        ...
        error_state_Product = true;
    }
    str1 += ' <div class="form-group"><label class="control-label col-sm-3" for="' + name + '">' + title + '</label><div class="col-sm-6">';
