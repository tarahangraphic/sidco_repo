var error_state_ProductSerial = false;
function creatmodalProductSerial(Content) {
...
function creatformProductSerial(content, listtag, listbuttons, title) {
    var modal = creatmodalProductSerial(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagProductSerial(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
    for (var j = 0; j < listbuttons.length; j++) {
        ...
        var id = '';
        if (j == 0) {
            id = 'btnregisterforProductSerial';
        }
        if (j == 1) {
            id = 'btnexit';
        }
...
function creattagProductSerial(type, title, name, event) {
