var error_state_UsersCar = false;
function creatmodalUserCar(Content) {
...
function creatformUserCar(content, listtag, listbuttons, title) {
    var modal = creatmodalUserCar(content);
    modal.Header.innerHTML += '<h4>' + title + '<h4>';
    for (var i = 0; i < listtag.length; i++) {
        modal.Body.innerHTML += creattagUserCar(listtag[i].type, listtag[i].title, listtag[i].name, listtag[i].event);
    }
...
function creattagUserCar(type, title, name, event) {
